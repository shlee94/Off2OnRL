...
def experiment(variant):
    ...
    if variant.get('pretrain_rl', False):
        ...
        policy_path = osp.join(logger.get_snapshot_dir(), 'model/policy.p')
        qf1_path = osp.join(logger.get_snapshot_dir(), 'model/qf1.p')
        qf2_path = osp.join(logger.get_snapshot_dir(), 'model/qf2.p')
        target_qf1_path = osp.join(logger.get_snapshot_dir(), 'model/target_qf1.p')
        target_qf2_path = osp.join(logger.get_snapshot_dir(), 'model/target_qf2.p')

        torch.save(policy.state_dict(), policy_path)
        torch.save(qf1.state_dict(), qf1_path)
        torch.save(qf2.state_dict(), qf2_path)
        torch.save(target_qf1.state_dict(), target_qf1_path)
        torch.save(target_qf2.state_dict(), target_qf2_path)
