    ...
    def compute_loss(
        ...
        if not skip_statistics:
            if not self.separate_buffers:
                ...
                eval_statistics["Online QF1 Loss"] = np.mean(
                    ptu.get_numpy(qf1_loss_[: batch_size // 2])
                )
                eval_statistics["Online QF2 Loss"] = np.mean(
                    ptu.get_numpy(qf2_loss_[: batch_size // 2])
                )
                eval_statistics["Offline QF1 Loss"] = np.mean(
                    ptu.get_numpy(qf1_loss_[batch_size // 2 :])
                )
                eval_statistics["Offline QF2 Loss"] = np.mean(
                    ptu.get_numpy(qf2_loss_[batch_size // 2 :])
                )
                ...
                eval_statistics.update(
                    create_stats_ordered_dict(
                        "Online Q1 Predictions",
                        ptu.get_numpy(q1_pred[: batch_size // 2]),
                    )
                )
                eval_statistics.update(
                    create_stats_ordered_dict(
                        "Online Q2 Predictions",
                        ptu.get_numpy(q2_pred[: batch_size // 2]),
                    )
                )
                eval_statistics.update(
                    create_stats_ordered_dict(
                        "Offline Q1 Predictions",
                        ptu.get_numpy(q1_pred[batch_size // 2 :]),
                    )
                )
                eval_statistics.update(
                    create_stats_ordered_dict(
                        "Offline Q2 Predictions",
                        ptu.get_numpy(q2_pred[batch_size // 2 :]),
                    )
                )
                ...
                eval_statistics.update(
                    create_stats_ordered_dict(
                        "Online Q Targets",
                        ptu.get_numpy(q_target[: batch_size // 2]),
                    )
                )
                eval_statistics.update(
                    create_stats_ordered_dict(
